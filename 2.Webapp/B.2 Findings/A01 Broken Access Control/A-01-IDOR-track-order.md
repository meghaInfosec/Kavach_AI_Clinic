...
```
`images/11-repeater-admin-order-exposed.png`
...
Screenshots 10 and 11 contain live JWT Bearer tokens issued during this lab session. Before publishing this write-up (or the `images/` folder) to a public GitHub repository, **redact/blur the `Authorization: Bearer ...` value** in both images — the tokens themselves are not needed to demonstrate the finding and should not be shared publicly, even from a disposable local lab account.
